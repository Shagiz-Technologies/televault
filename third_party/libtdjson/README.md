...
- Android `libtdjson.so` binaries for `armeabi-v7a`, `arm64-v8a`, and `x86_64` are stored under `android/src/main/jniLibs`. They are built from TDLib 1.8.66 commit `022d60202e446ad1287b9fb68e687c8a0760788b` using the pinned recipe at `tool/android/tdlib/Dockerfile`.
...
Android source/toolchain provenance and SHA-256 hashes are recorded in `android/TDLIB_BUILD_PROVENANCE.txt`. The release verifier checks the reviewed ELF alignment policy and GNU RELRO for every packaged ABI. Telegram login/upload behavior still requires release testing on physical Android and iOS devices.
