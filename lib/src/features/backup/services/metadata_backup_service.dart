      ...
      'files': files
          .map(
            (f) => {
              ...
              'iv_b64': f.ivB64,
              'vault_format_version': f.vaultFormatVersion,
              'encrypted_object_id': f.encryptedObjectId,
              'encrypted_size': f.encryptedSize,
              'original_size': f.originalSize,
              'vault_integrity_status': f.vaultIntegrityStatus,
              'vault_migration_status': f.vaultMigrationStatus,
              'key_wrapping_version': f.keyWrappingVersion,
              'last_verified_at': f.lastVerifiedAt?.toIso8601String(),
        ...
        await _db
            ...
            .insert(
              FilesCompanion.insert(
                ...
                vaultFormatVersion: Value(row['vault_format_version'] as int?),
                encryptedObjectId: Value(row['encrypted_object_id'] as String?),
                encryptedSize: Value(row['encrypted_size'] as int?),
                originalSize: Value(row['original_size'] as int?),
                vaultIntegrityStatus: Value(
                  row['vault_integrity_status'] as String? ?? 'unknown',
                ),
                vaultMigrationStatus: Value(
                  row['vault_migration_status'] as String? ?? 'notRequired',
                ),
                keyWrappingVersion: Value(row['key_wrapping_version'] as int?),
